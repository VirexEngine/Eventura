...
interface OrgJoinModalProps {
  isOpen: boolean;
  onClose: () => void;
  orgName: string;
}

export const OrgJoinModal = ({ isOpen, onClose, orgName }: OrgJoinModalProps) => {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        ...
        >
          ...
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-xl font-bold tracking-tight text-foreground">Join Organisation</h2>
          ...
          <div className="mb-6 pb-4 border-b border-border/50">
            <p className="text-sm text-primary font-semibold">Org Name: {orgName}</p>
          </div>
          ...
          <div className="space-y-4">
            <div className="tf">
              <div className="tf-label text-xs mb-1.5 font-medium flex items-center gap-1.5 text-muted-foreground">
                ...
                Specific Code <span className="text-live">*</span>
              </div>
            ...
            <div className="tf opacity-50 cursor-not-allowed">
              <div className="tf-label text-xs mb-1.5 font-medium flex items-center gap-1.5 text-muted-foreground">
                <span className="material-icons-round text-[14px]">person</span>
                Full Name <span className="opacity-50 font-normal">(auto-filled)</span>
              </div>
              <div className="relative group">
                <input
                  type="text"
                  value="Riya Sharma"
                  disabled
                  className="w-full bg-black/20 border border-primary/20 rounded-lg py-2.5 px-4 text-sm text-foreground focus:outline-none focus:border-primary/50 transition-colors cursor-not-allowed"
                />
              </div>
            </div>
          ...
          <div className="mt-8 flex items-center gap-3">
            ...
            <button
              onClick={() => {
                if (!specificCode || !orgCode) return alert("Specific Code and Organisation Code are required");
                onClose();
              }}
